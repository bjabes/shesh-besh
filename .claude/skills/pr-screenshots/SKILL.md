...
| `board-opponent-skip` | Mid-game state where the opponent rolls into a closed-out home and auto-skips. Captures the previous-roll tray slot showing the orphaned dice + "No legal moves" caption with "Your turn" on the right. | `SheshBesh/Shared/BoardView.swift` (TrayRow / PreviousRollSlot), `MatchViewModel.swift` (previousRollDice / previousRollSkipNote / phaseTitle), `Packages/SheshBeshGame/Sources/SheshBeshGame/MatchEngine.swift` (auto-skip capture), engine `previousTurn` plumbing |
